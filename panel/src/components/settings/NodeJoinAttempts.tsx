...
export default function NodeJoinAttempts({ onAdmitted }: { onAdmitted: () => void }) {
    ...
    const doApprove = async (a: NodeJoinAttempt) => {
        ...
        if (!(await confirmDialog({
            ...
            message: `Let "${name}" back in from ${a.peerIp}? The node's current key is replaced by a new one it generates on its next attempt, within about a minute, and its game servers are not restarted (a node whose pairing was reset, or one too old to have a key, gets a new secret instead, which does restart them). Approve this only if you recognise the machine and the address.`,
            confirmLabel: 'Admit',
            destructive: false,
        }))) return;
