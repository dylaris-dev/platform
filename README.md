...
- **Optional Gateway (separate repo).** Cross-repo communication is overwhelmingly through shared Redis keys/queues (e.g. `dylaris:hub:queue`, `route:{domain}`), with two exceptions. The Beam data plane (Beam client → relay → link → node) is a separate data path with its own wire protocol. And the gateway's **Warp client** and **kit Links** call Core's HTTP API directly — `POST /api/warp/enroll`, `GET /api/warp/assignment`, `POST /api/warp/link-boot`, and for a route-only Link, which has no Redis and no warp at all, `/api/warp/link/{edges,heartbeat,stats}` — so those host roles need HTTP reachability to Core.
...
| `BILLING_SUSPEND_GRACE` | `48h` | No | Grace before a `suspended` (BYON) tenant is cut off, so a transient billing/DB fault cannot instantly kick a paying customer. At the cutoff their servers stop, their route-only links' tunnels are taken down, **and their warp peers are removed, so the overlay tunnel itself goes down** — until then everything keeps running. Go duration; `0` enforces on the next hourly tick. |
